    ...
    items.forEach(function (item, index) {
        ...
        if (distanceX < 40 && distanceY < 40) {
            ...
            if (item.tipo === "cura") {
                ...
                addToInventory(item);
                console.log(
                    "🔑 CHAVE COLETADA! Agora você pode abrir a porta!"
                );
            } else {
                character.experiencia += item.valor;
                itemsCollected++;
...
function useAttack() {
    ...
    if (now - ability.lastUsed < ability.cooldown) {
        const timeLeft = Math.ceil(
            (ability.cooldown - (now - ability.lastUsed)) / 1000
        );
...
function startLevel(levelNumber) {
    ...
    document
        .querySelectorAll('.enemy, [id^="item-"], #exit-door')
...
function setupLevel3() {
    items = [{ x: 300, y: 100, tipo: "chave", emoji: "🔑", valor: 100 }];
    ...
    if (currentLevel === 3 && exitDoor) {
        ...
        if (distanceX < 40 && distanceY < 40) {
            if (hasKey) {
                ...
                alert(
                    "🚪 A porta está trancada! Você precisa encontrar a CHAVE 🔑 primeiro!"
                );
                console.log(
                    "⚠️ Procure pela chave 🔑 antes de tentar abrir a porta!"
                );
            }
        }
    }
...
function updateInfoPanel() {
    ...
    if (currentLevel === 1) {
        ...
        let timeLeft =
            level.timeLimit - Math.floor((Date.now() - levelStartTime) / 1000);
